...
test('Position iterator and JSON', t => {
	...
	if (!(Symbol.iterator in arrayPos)) {
		t.fail('position should have an iterator');
	}

	t.deepEqual([...arrayPos], [100, 200, 300], 'values from position\'s iterator should be equal to the array passed into its constrctor');

	t.deepEqual([...objectPos], [100, 200],	'positions created from objects should have an interator that returns the same values as the object\'s [x, y]');
...
test('Position aliases', t => {
	...
	t.equal(objectPos[0], 100, 'the x property in the position\'s constructor should be set as position[0]');
	t.equal(objectPos[1], 200, 'the y property in the position\'s constructor should be set as position[y]');
	t.end();
});
